...
class AddTrigger:
    ...
    def add_trigger(self, img):
        ...
        """
        # print(self.weight[self.weight>1])
...
class AddDatasetFolderTrigger(AddTrigger):
    ...
    def __call__(self, img):
        ...
        def add_trigger(img):
            ...
            #             print("this is pattern",self.pattern)
            #             test = res[res>0.15]
            #             print(sum(test))
            if img.dim() == 2:
                img = img.unsqueeze(0)
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(
        ...
        if specific_path is not None:
            ...
            self.poisoned_set = frozenset(
                [
                    i
                    for i, (img_path, _) in enumerate(benign_dataset.samples)
                    if img_path in path_set
                ]
            )
        ...
        self.poisoned_transform.transforms.insert(
            poisoned_transform_index, AddDatasetFolderTrigger(pattern, weight)
        )
        ...
        self.poisoned_target_transform.transforms.insert(
            poisoned_target_transform_index, ModifyTarget(y_target)
        )
