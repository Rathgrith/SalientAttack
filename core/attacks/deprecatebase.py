...
class Base(object):
    ...
    def checkCorrect_and_get_possibility(self,logit,label):
#         print(logit)
#         print(label)
        predicted_correct_index = torch.argmax(logit,dim=-1).item()
#         print(predicted_correct_index)
        
        true_correct_index = label.item()
        possibility = F.softmax(logit,dim=-1)
#         print(possibility[0][int(true_correct_index)])
        return predicted_correct_index==true_correct_index,possibility[0][int(true_correct_index)]
       

    
    
    def _test(self, dataset, device, batch_size=16, num_workers=8, model=None,image_title_dict=None):
        if model is None:
            model = self.model
        else:
            model = model
...
#                 print(image_base64)
#                 path = image_title_dict[image_base64]
#                 print(path)
                batch_img = batch_img.to(device)
                ...
                isCorrect,possibility= self.checkCorrect_and_get_possibility(batch_img,batch_label)
#                 print(isCorrect,possibility)
                if isCorrect:
                    correct_info_dict[path] = float(possibility)
                else: 
                    wrong_info_dict[path] = float(possibility)
            with open('Info_for_Correct_Model0.json','w') as f2:
                json.dump(correct_info_dict,f2)        
            with open('Info_for_Wrong_Model0.json', 'w') as f2:
                json.dump(wrong_info_dict,f2) 

            predict_digits = torch.cat(predict_digits, dim=0)
            labels = torch.cat(labels, dim=0)
            return predict_digits, labels

    def test(self, schedule=None, model=None, test_dataset=None, poisoned_test_dataset=None, image_title_dict=None):
        if schedule is None and self.global_schedule is None:
...
#             image_title_dict = self.current_schedule["image_title_dict"]
        ...
        if test_dataset is not None:
            last_time = time.time()
            # test result on benign test dataset
            predict_digits, labels= self._test(test_dataset, device, self.current_schedule['batch_size'], self.current_schedule['num_workers'], model,image_title_dict=image_title_dict)
            total_num = labels.size(0)
            prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
            top1_correct = int(round(prec1.item() / 100.0 * total_num))
            top5_correct = int(round(prec5.item() / 100.0 * total_num))
